import React from 'react';
import Category from './../../components/Category/Category';
import { WrapperButtonMore, WrapperCategory, WrapperProducts } from './style';
import SliderComponent from '../../components/SliderComponent/SliderComponent';
import slider1 from '../../assets/images/Slider1.webp';
import slider2 from '../../assets/images/Slider2.webp';
import slider3 from '../../assets/images/Slider3.webp';
import CardComponent from '../../components/CardComponent/CardComponent';


const HomePage = () => {
// fake category
const categpry = ['Quan ao', 'Dien Tu','Nha cua & doi song', 'My pham']
    return (
        <>
        <div style={{width:'1270px', margin:'0 auto'}}>
            <WrapperCategory>
        {categpry.map((item) => {
            return(
                <Category name= {item} key={item} />
            )
        })}
            </WrapperCategory>
            
        </div>
        <div id="body" style={{backgroundColor:"#efefef", width: '100%'}}>
        <div id="container" style={{ width: '1270px', height:'1000px', margin:'0 auto'}}>
            <SliderComponent arrImages ={[slider1, slider2, slider3]} />

            <WrapperProducts>
            <CardComponent /><CardComponent /><CardComponent /><CardComponent /><CardComponent /><CardComponent /><CardComponent /><CardComponent /><CardComponent /><CardComponent />
            </WrapperProducts>

            <div style={{width:'100%', display: 'flex',justifyContent:'center', marginTop:'10px'}}>
            <WrapperButtonMore textButton="Xem Them" type="outline" styleButton={{
            border: '1px solid rgb(11,116,229)',
            color: 'rgb(11,116,229)',
            width: '240px',
            height: '38px',
            borderRadius: '4px'
        }} 
            styleTextButton={{fontWeight: 600}}

        />
            </div>
        </div>
        </div>
        
        </>
    );
};

export default HomePage;